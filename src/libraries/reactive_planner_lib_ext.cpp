...
DiffeoTransformResult computeDiffeoTransform(
    ...
    std::vector<double> RobotPositionTransformed = {robot_position[0], robot_position[1]};
    ...
    for (size_t i = 0; i < diffeo_tree_array.size(); i++) {
        OutputStructVector TempTransformation = polygonDiffeoConvex(RobotPositionTransformed, diffeo_tree_array[i], diffeo_params);
    ...
    double alpha1 = -(RobotPositionTransformedD[1][0]*cos(robot_orientation) + RobotPositionTransformedD[1][1]*sin(robot_orientation));
    double beta1 = RobotPositionTransformedDD[0]*pow(cos(robot_orientation),2) + (RobotPositionTransformedDD[1]+RobotPositionTransformedDD[2])*sin(robot_orientation)*cos(robot_orientation) + RobotPositionTransformedDD[3]*pow(sin(robot_orientation),2);
    double alpha2 = RobotPositionTransformedD[0][0]*cos(robot_orientation) + RobotPositionTransformedD[0][1]*sin(robot_orientation);
    double beta2 = RobotPositionTransformedDD[4]*pow(cos(robot_orientation),2) + (RobotPositionTransformedDD[5]+RobotPositionTransformedDD[6])*sin(robot_orientation)*cos(robot_orientation) + RobotPositionTransformedDD[7]*pow(sin(robot_orientation),2);

    // Find transformed orientation
    double RobotOrientationTransformed = atan2(RobotPositionTransformedD[1][0]*cos(robot_orientation)+RobotPositionTransformedD[1][1]*sin(robot_orientation), RobotPositionTransformedD[0][0]*cos(robot_orientation)+RobotPositionTransformedD[0][1]*sin(robot_orientation));
